...
type RegistrarGateway interface {
	CreateNode(node types.NodeRegistrationRequest) (uint64, error)
	CreateTwin(relay string, pk []byte) (uint64, error)
	EnsureAccount(twinID uint64, pk []byte) (twin types.Account, err error)
	...
	UpdateNode(node types.NodeRegistrationRequest) (uint64, error)
